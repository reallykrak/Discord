    ...
    if (interaction.customId === 'create_ticket') {
      ...
      try {
        ...
        const ticketEmbed = new EmbedBuilder()
          ...
          .setDescription(`${interaction.user} tarafından oluşturuldu.`)
          .addFields(
            { name: 'Talep Sahibi', value: `${interaction.user.tag}` },
            { name: 'Kullanıcı ID', value: `${interaction.user.id}` },
        ...
        if (config.supportRoleId) {
          await channel.send({ content: `<@&${config.supportRoleId}> yeni bir destek talebi açıldı!` });
        }
